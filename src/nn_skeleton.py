...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    self.ph_lidar_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 7],
        name='lidar_input'
    )
    ...
    self.q = tf.FIFOQueue(
        ...
        shapes=[[],
                [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 7],
                [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL],
                [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL]]
    )
    self.enqueue_op = self.q.enqueue(
        [self.ph_keep_prob, self.ph_lidar_input,
          self.ph_label, self.ph_loss_weight]
    )

    self.keep_prob, self.lidar_input, self.label, \
  ...
  def _add_output_graph(self):
    ...
    with tf.variable_scope('interpret_output') as scope:
      self.prob = tf.nn.softmax(self.output_prob, dim=-1, name='pred_prob')
  ...
  def _conv_bn_layer(
      ...
      size, stride, padding='SA'
                            'ME', freeze=False, relu=True,
  ...
  def _pooling_layer(
      self, layer_name, inputs, size, stride1,stride2, padding='SAME'):

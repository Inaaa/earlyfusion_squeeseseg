...
def train():
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.net == 'squeezeSeg':
      ...
      model = SqueezeSeg(mc)                           # define the network
    ...
    def enqueue(sess, coord):  ## get input data
      with coord.stop_on_exception():
        while not coord.should_stop():
          # read batch input
          image_lidar_per_batch, label_per_batch,\
              weight_per_batch = imdb.read_batch()

          feed_dict = {
              model.ph_keep_prob: mc.KEEP_PROB,
              model.ph_lidar_input: image_lidar_per_batch,
    ...
    saver = tf.train.Saver(tf.all_variables())   #Add ops to save and restore all the variables.
    summary_op = tf.summary.merge_all()  ## save all summary and can be showed in tensorflow
    ...
    try:
      for step in xrange(FLAGS.max_steps):
        ...
        if step % FLAGS.summary_step == 0 or step == FLAGS.max_steps-1:
          op_list = [
              model.lidar_input, model.label, model.train_op,
              model.loss, model.pred_cls, summary_op
          ]
          #run the network
          lidar_per_batch,  label_per_batch, \
              _, loss_value, pred_cls, summary_str = sess.run(op_list,
                                                              options=run_options)  ###already give the data in the enqueue
          print('lidar_per_batch_shape {}'.format(lidar_per_batch.shape))

          label_image = visualize_seg(label_per_batch[:7, :, :], mc)
          pred_image = visualize_seg(pred_cls[:7, :, :], mc)
          ...
          viz_summary_list = sess.run(
              viz_op_list, 
              feed_dict={
                  model.depth_image_to_show: lidar_per_batch[:1, :, :, [6]],
        ...
        assert not np.isnan(loss_value), \
            ...
            'class_loss: {}'.format(loss_value, conf_loss, bbox_loss,
                                    class_loss)Timed out waiting for notification
        '''
